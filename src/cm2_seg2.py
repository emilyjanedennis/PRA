...
if __name__ == "__main__":
	...
	for i in np.arange(0,np.shape(cells_floored)[1]):
		...
		try:
			...
			fails+=1
	sigma_vals_for_bool = np.multiply(bool_cell_vol,ann_volume) 
	print("{} failed points outside volume space".format(fails))

	tif.imsave(os.path.join(save_dir,"{}_{}_masked_bool_cells_in_sigma.tif".format(brain_name,ch)),sigma_vals_for_bool)
	...
	ann_labels.to_csv(os.path.join(save_dir,"{}_{}_labels.csv".format(brain_name,ch)))
