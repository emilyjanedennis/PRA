...
import tifffile
from utils.io import listdirfull, makedir, removedir, chunkit, writer
from utils.io import load_kwargs, load_tif_list, save_kwargs
...
class volume:
    ...
    # location(s)*** for downsized folder or file, downsized 2d cell file will store here
    def add_downsized_vol(self, loc):
...
def regex_determiner(raw, dr):
    ...
    if raw == True:
        ...
        fl = [f for f in os.listdir(dr) if "raw_DataStack" in f or "raw_RawDataStack" in f]
    elif raw == False:
        fl = [f for f in os.listdir(
            dr) if "raw_DataStack" not in f and ".tif" in f or "raw_RawDataStack" in f]
    # determine regex
...
def generateparamdict(cwd, dst=False, update=False, **kwargs):
    """Function to automatically determine structure of light sheet scan,
    including channels, light sheets, tiles etc.
    This feeds the rest of the pipeline.
    _______
    Inputs:
        dst = (optional), set to name of param dictionary: e.g.
        "param_dict_local.p"; useful to make several param dicts
        each with a different cwd allowing for work on local machine
        and clusters
        Update(optional) set to False to force complete regeneration
        of param dict

    """
    # inputs
    kwargscopy = kwargs.copy()
    # the above is done to get rid of the systemdirectory
    outdr = kwargs["outputdirectory"]
    raw = kwargs["rawdata"]
    kwargs["packagedirectory"] = cwd
    makedir(os.path.join(cwd, "logs"))
    if not "parameterfolder" in kwargs:
    ...
    if os.path.exists(kwargs["outputdirectory"]+"/param_dict.p") and update:
        ...
        for pth, lst in kwargs["inputdictionary"].items():
            ...
            for i in lst:  # i=["regch", "00"]
                ...
                makedir(os.path.join(outdr, "injection"))
                makedir(vol.injcoordinatesfld)
                vol.add_injdetect3dfld(os.path.join(outdr, "injection", "injdetect3d"))
                makedir(vol.injdetect3dfld)
                vol.add_allen_id_table(os.path.join(
                    kwargs["packagedirectory"], "supp_files/allen_id_table.xlsx"))
                vol.add_full_sizedatafld(os.path.join(outdr, "full_sizedatafld"))
                vol.add_full_sizedatafld_vol(os.path.join(
                    vol.full_sizedatafld, "{}_ch{}".format(vol.brainname, vol.channel)))
                vol.add_regex(regex)
                ...
                vol.add_reg_to_atlas_vol(outdr+"/elastix/result.{}.tif".format(
                    len([xx for xx in os.listdir(kwargs["parameterfolder"]) if "Order" in xx and "~" not in xx])-1))
                vol.add_reg_to_atlas_transform(outdr+"/elastix/TransformParameters.{}.txt".format(
                    len([xx for xx in os.listdir(kwargs["parameterfolder"]) if "Order" in xx and "~" not in xx])-1))
                secondary_registration = kwargs["secondary_registration"] if "secondary_registration" in kwargs else True
                if not secondary_registration:
                    vol.add_ch_to_reg_to_atlas(outdr+"/elastix/{}_resized_ch{}/result.{}.tif".format(vol.brainname, vol.channel, len(
                        [xx for xx in os.listdir(kwargs["parameterfolder"]) if "Order" in xx and "~" not in xx])-1))
                ...
                try:
                    ...
                    kwargs["parameterfolder"] = os.path.join(
                        kwargs["packagedirectory"], "parameterfolder")
                    vol.add_parameterfolder(kwargs["parameterfolder"])
                ...
                tmpzdct = {}
                [tmpzdct.update(dict([(keys, dict([(k, v)]))])) for keys, values in tmpdct["zchanneldct"].items(
                ) for k, v in values.items() if int(i[1]) == int(k)]
                if len(tmpzdct) == 0:
                    print("zdct contains 0 planes - this will result in an error for stitching")
                vol.add_zdct(tmpzdct)
                writer(outdr, "\nGenerated volume class('{}') channel('{}') pth: {}\n".format(
                    vol.ch_type, vol.channel, vol.indr))
        ...
        if not dst:
            dst = kwargs["outputdirectory"]+"/param_dict.p"
...
def updateparams(cwd=False, **kwargs):
    ...
    outdr = kwargs["outputdirectory"]
    raw = kwargs["rawdata"]
    if not "packagedirectory" in kwargs:
        kwargs["packagedirectory"] = os.getcwd()
    if not "parameterfolder" in kwargs:
        kwargs["parameterfolder"] = os.path.join(kwargs["packagedirectory"], "parameterfolder")
    ############# make output directory #############
    makedir(outdr[:outdr.rfind("/")])  # in case of nested folders
...
def zchanneldct(dr=None, **kwargs):
    ...
    ****function filters out nonraw data****
    ...
    chs = []
    [chs.append(matches[i].group("ch"))
     for i in range(len(matches)) if matches[i].group("ch") not in chs]
    # make dct consisting of each channel sorted by z plane and then in xy order (topleft-->top right to bottom-right)
    if tiling:
        for ch in chs:
            lst = []
            try:
                [lst.append("".join(match.groups()))
                 for num, match in enumerate(matches) if ch in match.group("ch")]
            except:
                pass
            srtd = sorted(lst, key=lambda a: (
                a[z_indx[0]:z_indx[1]], a[y_indx[0]:y_indx[1]], a[x_indx[0]:x_indx[1]]))  # sort by z, then x, then y
            ytile = int(max([yy for f in lst for yy in f[y_indx[0]:y_indx[1]]])) + \
                1  # automatically find the number of tiles
            xtile = int(max([xx for f in lst for xx in f[x_indx[0]:x_indx[1]]])) + \
                1  # automatically find the number of tiles
            intvl = xtile*ytile
            # each zplane in sequence of x positions then y position Top Left to bottom right
            grpd = [srtd[itr*intvl:(intvl*(itr+1))] for itr in range(int(len(srtd)/intvl))]
            dct[ch] = grpd
    ...
    try:
        ...
        lsheets = []
        [lsheets.append(matches[i].group("ls")[-2:])
         for i in range(len(matches)) if matches[i].group("ls")[-2:] not in lsheets]
        lsheets = int(max([lsh for f in lst for lsh in f[ls_indx[0]:ls_indx[1]]])
                      )+1  # automatically find the number of light sheets
    except IndexError:
        lsheets = 1
    # New find HF:
...
def arrayjob(jobid, cores=5, compression=0, verbose=True, **kwargs):
    ...
    # submits sequential jobs in range starting at arrayid for length slurmfactor
    [process_planes(job, cores, compression, verbose, **kwargs)
...
def process_planes(job, cores, compression, verbose=True, **kwargs):
    ...
    # default to uint16 unless specified
    bitdepth = kwargs["bitdepth"] if "bitdepth" in kwargs else "uint16"
    ####################################

    for i, vol in enumerate(kwargs["volumes"]):
        if i == 0:
            writer(vol.full_sizedatafld, "Processing zpln {}...".format(
                zpln), flnm="process.txt", verbose=verbose)
        try:
        ...
        if vol.raw:
            stitchdct = flatten_stitcher(cores, vol.outdr, vol.tiling_overlap,
                                         vol.xtile, vol.ytile, zpln, dct, vol.lightsheets, **kwargs)
        if not vol.raw:
        ...
        resize_save(cores, stitchdct, vol.outdr, resizefactor, vol.brainname, zpln, bitdepth)
        writer(vol.full_sizedatafld, "\n   completed zpln {}, channel {}, channel_type {}, nm {}".format(
            zpln, vol.channel, vol.ch_type, os.path.basename(vol.full_sizedatafld_vol)), flnm="process.txt", verbose=verbose)
...
def process_planes_from_fullsizedatafolder(job, cores, compression, verbose=True, **kwargs):
    ...
    if cores < 2:
        ...
        for vol in kwargs["volumes"]:
            ...
            if len(fl) == 1:
                resize_save_helper(vol.outdr, kwargs["resizefactor"], vol.brainname, zpln, vol.channel,
                                   tifffile.imread(fl[0]).astype(bitdepth), bitdepth)
    else:
        p = mp.Pool(cores)
        for vol in kwargs["volumes"]:
            fl = [xx for xx in listdirfull(vol.full_sizedatafld_vol, "tif")]
            fl.sort()
            zplns = [str(job).zfill(4) for job in range(len(fl))]
            iterlst = []
            [iterlst.append((vol.outdr, kwargs["resizefactor"], vol.brainname, zpln,
    ...
    for vol in kwargs["volumes"]:
        try:
            ...
            if len(listdirfull(vol.full_sizedatafld_vol, keyword=".tif")) != zmax or len(find_outlier_files_from_size(listdirfull(vol.full_sizedatafld_vol, keyword=".tif"))) > 0:
                writer(outdr, "STEP 1: FAILED: {} files found in {}. Should have {}.\n".format(len(listdirfull(
                    vol.full_sizedatafld_vol, keyword=".tif")), vol.full_sizedatafld_vol[vol.full_sizedatafld_vol.rfind("/")+1:], zmax))
                completed = [int(xx[-8:-4]) for xx in listdirfull(vol.full_sizedatafld_vol,
                                                                  keyword=".tif") if int(xx[-8:-4]) in range(zmax)]
                missing = list(set(range(zmax)).difference(
                    set(completed)))+find_outlier_files_from_size(listdirfull(vol.full_sizedatafld_vol, keyword=".tif"))
                tick = 0
                ...
                writer(outdr, "\n\n***STEP 1: Correct number({}) of files found for {}.**\n\n".format(len(listdirfull(
                    vol.full_sizedatafld_vol, keyword=".tif")), vol.full_sizedatafld_vol[vol.full_sizedatafld_vol.rfind("/")+1:]))
        except Exception as e:
...
def find_outlier_files_from_size(src, deviation=5):
    ...
    mean = np.mean(arr)
    std = np.std(arr)
    lst = [fl for fl in src if os.stat(fl).st_size > (
        mean+deviation*std) or os.stat(fl).st_size < (mean-deviation*std)]
    return lst
...
def combiner_completionchecker(**kwargs):
    ...
    ############################
    allchns = {}
    allchns.update(resizedregdct)
    allchns.update(resizedsigdct)
    chnlst = []
    missing_chn = []
    for ch in allchns.keys():
        try:
            with open(os.path.join(outdr, "LogFile.txt"), "r") as f:
                ...
                s.seek(s.find(str(ch)+" in"))
                chn = s.readline()
                chn = chn[:chn.rfind(" in")]
                s.close()
            chnlst.append(chn)
        except ValueError:
...
def resize_save(cores, stitchdct, outdr, resizefactor, brainname, zpln, bitdepth):
    ...
    if cores > 1:
        try:
            ...
            p = mp.Pool(cores)
        iterlst = []
        [iterlst.append((outdr, resizefactor, brainname, zpln, ch, im, bitdepth))
         for ch, im in stitchdct.items()]
        svloc = p.starmap(resize_save_helper, iterlst)
        svloc.sort()
        p.terminate()
    else:
        for ch, im in stitchdct.items():
...
def resize_save_helper(outdr, resizefactor, brainname, zpln, ch, im, bitdepth):
    ...
    im1 = cv2.resize(im, (xfct, yfct), interpolation=cv2.INTER_LINEAR)
...
def saver(cores, stitchdct, outdr, brainname, zpln, compression, bitdepth):
    ...
    if cores > 1:
        try:
            ...
            p = mp.Pool(cores)
        iterlst = []
        [iterlst.append((outdr, brainname, zpln, ch, im, compression, bitdepth))
         for ch, im in stitchdct.items()]
        lst = p.starmap(saver_helper, iterlst)
        lst.sort()
        p.terminate()
        del p
    else:
...
def saver_helper(outdr, brainname, zpln, ch, im, compression, bitdepth):
    ...
    tifffile.imsave(pth, im.astype(bitdepth), compress=compression)
    del im
    return pth
...
def stitcher(cores, outdr, ovlp, xtile, ytile, zpln, dct, blndtype=False, intensitycorrection=False, **kwargs):
    ...
    ["stitching for ch_{}".format(ch) for ch, zplnlst in dct.items()
     ]  # cheating way to set ch and zplnlst
    ...
    # check for inputs:
    if not blndtype and "blendtype" in kwargs:
        blndtype = kwargs["blendtype"]
    if not blndtype and "blendtype" not in kwargs:
        blndtype = "sigmoidal"
    if not intensitycorrection:
...
# blending setup
    if blndtype == "linear":
        # might need to change 0-1 to 0-255?
        alpha = np.tile(np.linspace(0, 1, num=xpxovlp), (ydim, 1))
        yalpha = np.swapaxes(np.tile(np.linspace(0, 1, num=int(ypxovlp)),
                                     (int(xdim+((1-ovlp)*xdim*(xtile-1))), 1)), 0, 1)

    elif blndtype == "sigmoidal":
        alpha = np.tile(scipy.stats.logistic.cdf(np.linspace(-blendfactor, blendfactor,
                                                             num=xpxovlp)), (ydim, 1))  # might need to change 0-1 to 0-255?
        yalpha = np.swapaxes(np.tile(scipy.stats.logistic.cdf(
            np.linspace(-blendfactor, blendfactor, num=int(ypxovlp))), (int(xdim+((1-ovlp)*xdim*(xtile-1))), 1)), 0, 1)

    # No blending: generate np array with 0 for half of overlap and 1 for other.
    elif blndtype == False or blndtype == None:
        alpha = np.zeros((ydim, xpxovlp))
        alpha[:, int(xpxovlp/2):] = 1
        yalpha = np.zeros((ypxovlp, (xdim+((1-ovlp)*xdim*(xtile-1)))))
        yalpha[int(ypxovlp/2):, :] = 1
    ...
    iterlst = [(xdim, ydim, xtile, ytile, ovlp, xpxovlp, ypxovlp, tiles, alpha, yalpha,
                zpln, ch, zplnlst, intensitycorrection) for ch, zplnlst in dct.items()]
    # parallel processing
    # OFF RIGHT NOW
    if False:  # cores>1:
        ...
        while ytick < ytile:
            while xtick < xtile:
                ...
                if xtick != 0:
                    ...
                    l_im_ovlp = xfrm[:, -(xpxovlp):]
                    r_im_ovlp = im[:, :(xpxovlp)]
                    #pl.figure(); pl.hist(l_im_ovlp.ravel(), bins=100, alpha=0.5); pl.hist(r_im_ovlp.ravel(), bins=100, alpha=0.5)
                    l_im_mean = np.mean(l_im_ovlp.ravel())
                    r_im_mean = np.mean(r_im_ovlp.ravel())
                    mean_dif = abs(r_im_mean - l_im_mean)
                    ...
                    if l_im_mean < r_im_mean:
                        ...
                        xfrm = xfrm - mean_dif
                        xfrm = xfrm.clip(min=0)
                    # stitch
                    xblend = (xfrm[:, -(xpxovlp):]*(1-alpha) + im[:, :(xpxovlp)]
                              * (alpha)).astype("uint16")  # 1-alhpa alpha
            ...
            if ytick != 0:
                # correct for overall image intensity difference by shifting higher mean overlapping pixels towards smaller
                t_im_ovlp = xyfrm[-(ypxovlp):, :]
                b_im_ovlp = xfrm[:ypxovlp, :]
                #pl.hist(t_im_ovlp.ravel(), bins=100, alpha=0.5); pl.hist(b_im_ovlp.ravel(), bins=100, alpha=0.5)
                t_im_mean = np.mean(t_im_ovlp.ravel())
                b_im_mean = np.mean(b_im_ovlp.ravel())
                mean_dif = abs(t_im_mean - b_im_mean)
                # move the means two images closer to each other
                if t_im_mean < b_im_mean:
                    xfrm = xfrm-mean_dif
                    xfrm = xfrm.clip(min=0)
                elif b_im_mean < t_im_mean:
                    xyfrm = xyfrm - mean_dif
                    xyfrm = xyfrm.clip(min=0)
                # stitch
                yblend = (xyfrm[-(ypxovlp):, :]*(1-yalpha) +
                          xfrm[:(ypxovlp), :] * yalpha).astype("uint16")
...
def resample_par(cores, mv_pth, fx_pth=None, size=None, svlocname=None, singletifffile=None, resamplefactor=None):
   ...
   -------------------------
    ...
    if resamplefactor != None:
        x = int(float(resamplefactor)*float(x))
        y = int(float(resamplefactor)*float(y))
        z = int(float(resamplefactor)*float(z))
...
################# chunk then resize in xy ########################
    if not singletifffile:
        ...
        movingimage_list_full = [f for f in listdirfull(mv_pth) if ".tif" in f]
        movingimage_list_full.sort()
        sys.stdout.write(
            "Running resizing with {} cores\n  Dims {} -> {}\n".format(cores, (xmov, ymov, zmov), (x, y, z)))
        sys.stdout.flush()
        iterlst = []
        [iterlst.append((core, cores, x, y, movingimage_list_full, tmpsvloc))
         for core in range(cores)]
        lst = p.starmap(xyresizer, iterlst)
        lst.sort()
        p.terminate()
    elif singletifffile:
        # find size of mv_pth
        ...
        sys.stdout.flush()
        iterlst = []
        [iterlst.append((core, cores, x, y, mv_pth, zmov, tmpsvloc)) for core in range(cores)]
        lst = p.starmap(xyresizer2, iterlst)
        lst1 = []
        [lst1.append(xx) for xx in lst if xx not in lst1]
        del lst
        lst = lst1
        lst.sort()
    print("Completed XY resizing in {} seconds, starting to resize in Z...".format(
...
##########swapaxes to yzx, save then resize in xz #####################
    ...
    iterlst = []
    [iterlst.append((core, cores, xstack, z, tmpfl, ystack, tmpsvloc)) for core in range(cores)]
    lst = p.starmap(xyresizer2, iterlst)
    lst1 = []
    [lst1.append(xx) for xx in lst if xx not in lst1]
    del lst
...
################### output or save ################
    if svlocname == None:
        print("Total time for XY and Z resizing: {} seconds.\nFile NOT SAVED. IF DESIRED make svlocname='desiredname'".format(
            (time.time() - total_time), os.path.join(os.getcwd())))
        return stack
    elif svlocname != None:
        if svlocname[-4:] == ".tif":
        ...
        print("Total time for XY and Z resizing: {} seconds.\nFile saved as {}".format(
            (time.time() - total_time), os.path.join(os.getcwd(), svlocname)))
        return
...
def xyresizer(core, cores, x, y, movingimage_list_full, tmpsvloc):
    ...
    for zpln in range(len(movingimage_list_full)):
        im = image[zpln, ...]
        imstack[zpln, ...] = cv2.resize(
            im, (x, y), interpolation=cv2.INTER_LINEAR)  # cv2.INTER_AREA)
    del image
...
def color_movie_merger(movie2, movie4, savelocation=None, savefilename=None, movie5=None):
    ...
    if savelocation != None:
        ...
        tifffile.imsave(os.path.join(savelocation, "combinedmovies",
                                     "Merged_{}.tif".format(savefilename)), colorfile)
...
# ensure proper input to function
    ...
    if singletifffile == None:
        ...
        movingimage_list = [f for f in os.listdir(
            mv_pth) if os.path.isfile(os.path.join(mv_pth, f))]
        ...
        for files in movingimage_list_full:
            ...
            # interpolation=cv2.INTER_AREA) #cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA
            ...
            ticker += 1
            if verbose:
                print("Resized slice {} of {} to {}x{} pixels in {} seconds".format(
                    ticker, length, x, y, (time.time()-timer)))
        print("Completed XY resizing in {} seconds, starting to resize in Z...".format(
            (time.time() - total_time)))
        ...
        for files in range(zmov):
            ...
            if verbose:
                print("Resized slice {} of {} to {}x{} pixels in {} seconds".format(
                    ticker, zmov, x, y, (time.time()-timer)))
        print("Completed XY resizing in {} seconds, starting to resize in Z...".format(
            (time.time() - total_time)))
# need to figure out z now....
    ...
    for xplane in filestack:  # YXZ
        timer = time.time()
        # interpolation=cv2.INTER_AREA) #resize in xz
        image = cv2.resize(xplane, (z, x), interpolation=cv2.INTER_LINEAR)
        try:
        ...
        ticker += 1
        if verbose:
            print("Z Resizing {} of {} to {}x{} pixels in {} seconds".format(
                ticker, y, x, z, (time.time()-timer)))
...
def tiffcombiner(jobid, cores=None, remove=True, save_before_reorientation=False, **kwargs):
    ...
    if jobid >= len(vols):
        ...
        return
    lst = []
    [lst.append(vol.downsized_vol) for vol in vols]
    ...
    if save_before_reorientation:
        z, y, x = imstack.shape
        fp = np.lib.format.open_memmap(vol_to_process[:vol_to_process.rfind(
            "/")]+"tiffcombiner_before_reorientation.npy", dtype="uint16", mode="w+", shape=(z, y, x))
        fp = imstack
        sys.stdout.write("\n     Saving image before reorientation as: {}".format(
            vol_to_process[:vol_to_process.rfind("/")]+"tiffcombiner_before_reorientation.npy"))
...
def summarycomparision(dr1, dr2, svloc, dr1search=None, dr2search=None):
    ...
    for names in longdict.values()[0]:
        ...
        if [filename for subnames in fls if filename in subnames] == [filename for subnames in fll if filename in subnames]:
            ...
            cv2.putText(im1, longdict.keys()[0][longdict.keys()[
                        0].rfind("/")+1:], (10, 30), font, 1, color, 2)
            cv2.putText(im1, metricfinder(
                [metric for metric in efll if filename in metric][0]), (470, 30), font, 0.6, color, 2)
            im2 = cv2.imread([sbnm for sbnm in fls if filename in sbnm][0])
            cv2.putText(im2, shortdict.keys()[0][shortdict.keys()[0].rfind(
                "/")+1:], (10, 30), font, 1, color, 2)  # , cv2.LINE_AA)
            cv2.putText(im2, metricfinder(
                [metric for metric in efls if filename in metric][0]), (470, 30), font, 0.6, color, 2)
            im12 = np.concatenate((im1, im2), axis=1)
            cv2.putText(im12, names[names.rfind("DREADDs/"):names.rfind("/" +
                                                                        searchfor1)], (100, 535), font, 1.5, color, 2)  # , cv2.LINE_AA)
            try:
                stack[tick, ...] = im12
            except NameError:
                y, x, c = im12.shape
    ...
    if badlst != 0:
        for badim in badlst:
            ...
            cv2.putText(badim1, longdict.keys()[0][longdict.keys()[
                        0].rfind("/")+1:], (10, 30), font, 1, color, 2)
            cv2.putText(badim1, metricfinder(
                [metric for metric in efll if filename in metric][0]), (470, 30), font, 0.6, color, 2)
    ...
    tifffile.imsave(os.path.join(svloc, dr1[dr1.rfind("/")+1:] +
                                 "_w_"+dr2[dr2.rfind("/")+1:]+".tif"), stack.astype("uint16"))
    print("Saved {}".format(os.path.join(
        svloc, dr1[dr1.rfind("/")+1:]+"_w_"+dr2[dr2.rfind("/")+1:]+".tif")))
...
def summarycombine(dr1, svlocname, filenamesearch=None):
    ...
    for names in fl1:
        ...
        cv2.putText(im1, names[names.rfind(dr1[-3:]) +
                               4:names.rfind("/"+searchfor)], (10, 535), font, 1.5, color, 2)
        cv2.putText(im1, metricfinder(
            [metric for metric in efl if filename in metric][0]), (470, 30), font, 0.6, color, 2)
        try:
            stack[tick, ...] = im1
        except NameError:
            y, x, c = im1.shape
...
def metricfinder(dr):
    ...
    f = open(dr, "r")
    s = mmap.mmap(f.fileno(), 0, access=mmap.ACCESS_READ)
    s.seek(s.rfind("Final metric value"))
    metric = s.readline()
    metric = metric[:metric.rfind("\\")]
    s.close()
    return metric
...
def gridlineresizer(mv, **kwargs):
   ...
   # preprocess gridline file: Correcting for z difference between gridline file and atlas
    # interpolation=cv2.INTER_CUBIC) # #cv2.INTER_LINEAR, cv2.INTER_CUBIC, cv2.INTER_AREA
    grdresized = cv2.resize(grdim, (x, y), interpolation=cv2.INTER_LINEAR)
...
def combine_images(movie1, movie2, movie3, movie4, svlc, brainname):
    ...
    m2 = sitk.GetArrayFromImage(sitk.RescaleIntensity(sitk.ReadImage(movie2)))
    # don"t need to rescale gridline filter
    m3 = sitk.GetArrayFromImage(sitk.RescaleIntensity(sitk.ReadImage(movie3)))
    m4 = sitk.GetArrayFromImage(sitk.RescaleIntensity(sitk.ReadImage(movie4)))
...
#        m1_taller=np.hstack((np.))
    ...
    tifffile.imsave(os.path.join(svlc, "combinedmovies",
                                 "Concatenated_{}.tif".format(savefilename)), m1234.astype(np.uint8))
...
def flatten(dr, **kwargs):
    ...
    [chs.append(matches[i].group("ch")[:])
     for i in range(len(matches)) if matches[i].group("ch")[:] not in chs]
    zplns = []
    [zplns.append(matches[i].group("z"))
     for i in range(len(matches)) if matches[i].group("z") not in zplns]
    zplns.sort()
    # zmx=max(zplns)
    with tifffile.TiffFile(os.path.join(dr, "".join(matches[0].groups()))) as tif:
        if len(tif.pages[0].shape) == 3:
            hf, y, x = tif.pages[0].shape
    ...
    for ch in chs:
        ...
        try:
            ...
            [lst.append("".join(match.groups()))
             for num, match in enumerate(matches) if ch in match.group("ch")]
        try:
            srtd = sorted(lst, key=lambda a: (a[z_indx[0]:z_indx[1]],  a[ls_indx[0]:ls_indx[1]],
                                              a[y_indx[0]:y_indx[1]], a[x_indx[0]:x_indx[1]]))  # sort by z, then ls, then x, then y
        except NameError:
            srtd = sorted(lst, key=lambda a: (
                a[z_indx[0]:z_indx[1]],  a[y_indx[0]:y_indx[1]], a[x_indx[0]:x_indx[1]]))  # sort by z, then x, then y
        if tiling:
        ...
        try:
            ...
            [lsheets.append(matches[i].group("ls")[-2:])
             for i in range(len(matches)) if matches[i].group("ls")[-2:] not in lsheets]
            # automatically find the number of light sheets
            lsheets = int(max([lsh for f in lst for lsh in f[ls_indx[0]:ls_indx[1]]]))+1
        ...
        for plns in zplns:
            try:
                tmp = []
                [tmp.append(xx) for xx in srtd if "Z"+plns in xx]
                tmp = sorted(tmp, key=lambda a: (
                    a[z_indx[0]:z_indx[1]],  a[ls_indx[0]:ls_indx[1]], a[y_indx[0]:y_indx[1]], a[x_indx[0]:x_indx[1]]))
            except NameError:
                tmp = []
                [tmp.append(xx) for xx in srtd if "Z"+plns in xx]
                tmp = sorted(tmp, key=lambda a: (
                    a[z_indx[0]:z_indx[1]],  a[y_indx[0]:y_indx[1]], a[x_indx[0]:x_indx[1]]))
            ttdct[plns] = tmp
...
def flatten_stitcher(cores, outdr, ovlp, xtile, ytile, zpln, dct, lightsheets, **kwargs):
    ...
    # easy way to set ch and zplnlst
    ["stitching for ch_{}".format(ch[-2:]) for ch, zplnlst in dct.items()
     ]  # cheating way to set ch and zplnlst
    ...
    if blndtype == "linear":
        # might need to change 0-1 to 0-255?
        alpha = np.tile(np.linspace(0, 1, num=int(xpxovlp)), (ydim, 1))
        yalpha = np.swapaxes(np.tile(np.linspace(0, 1, num=int(ypxovlp)),
                                     (int(xdim+((1-ovlp)*xdim*(xtile-1))), 1)), 0, 1)
    elif blndtype == "sigmoidal":
        alpha = np.tile(scipy.stats.logistic.cdf(np.linspace(-blendfactor, blendfactor,
                                                             num=int(xpxovlp))), (ydim, 1))  # might need to change 0-1 to 0-255?
        yalpha = np.swapaxes(np.tile(scipy.stats.logistic.cdf(
            np.linspace(-blendfactor, blendfactor, num=int(ypxovlp))), (int(xdim+((1-ovlp)*xdim*(xtile-1))), 1)), 0, 1)
    # No blending: generate np array with 0 for half of overlap and 1 for other.
    elif blndtype == False or blndtype == None:
        alpha = np.zeros((ydim, xpxovlp))
        alpha[:, xpxovlp/2:] = 1
        yalpha = np.zeros((int(ypxovlp), int((xdim+((1-ovlp)*xdim*(xtile-1))))))
        yalpha[int(ypxovlp/2):, :] = 1
    else:
        alpha = np.tile(scipy.stats.logistic.cdf(np.linspace(-blendfactor, blendfactor,
                                                             num=int(xpxovlp))), (ydim, 1))  # might need to change 0-1 to 0-255?
    ...
    if len(dct) <= 1:
        stitchlst = [flatten_xystitcher(xdim, ydim, xtile, ytile, ovlp, xpxovlp, ypxovlp, tiles, alpha,
                                        yalpha, zpln, ch[-2:], zplnlst, intensitycorrection, lightsheets) for ch, zplnlst in dct.items()]
    else:
        # parallel processing
        p = mp.Pool(cores)
        iterlst = [(xdim, ydim, xtile, ytile, ovlp, xpxovlp, ypxovlp, tiles, alpha, yalpha, zpln,
                    ch[-2:], zplnlst, intensitycorrection, lightsheets) for ch, zplnlst in dct.items()]
        try:
        ...
        del iterlst
    stitchdct = {}
    [stitchdct.update(i) for i in stitchlst]
...
def flatten_xystitcher(xdim, ydim, xtile, ytile, ovlp, xpxovlp, ypxovlp,
    ...
    # cleaned up 201802 - if breaks change imread as float64
    lst = [tifffile.imread("".join(im), multifile=False) for im in zplnlst]

    # max project HF
    if len(lst[0].shape) > 2:
        lst = [np.amax(xx, 0) for xx in lst]
        ...
        for lst in lsts:
            ...
            while ytick < ytile:
                while xtick < xtile:
                    ...
                    if xtick != 0:
                        ...
                        if l_im_mean < r_im_mean:
                            ...
                            xfrm = xfrm.clip(min=0)
                        # stitch
                        xblend = (xfrm[:, -(xpxovlp):]*(1-alpha) +
                                  im[:, :(xpxovlp)] * (alpha))  # 1-alhpa alpha
                        xfrm = np.concatenate(
                            (xfrm[:, :-(xpxovlp)], xblend, im[:, xpxovlp:]), axis=1)
                    else:
                        xfrm = im
                    xtick += 1
                xtick = 0
                if ytick != 0:
                    ...
                    xyfrm = np.concatenate(
                        (xyfrm[:-(ypxovlp), :], yblend, xfrm[ypxovlp:, :]), axis=0)
                else:
                    xyfrm = xfrm
                ytick += 1
