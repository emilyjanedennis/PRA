    ...
    for prefix in ["/jukebox/", "/mnt/bucket/labs/", "/home/ejdennis/", "/scratch/gpfs/ejdennis/", "/tigress/ejdennis/", "/home/emilyjanedennis/"]:
        if type(item) == dict:
            for keys, values in item.items():
                if type(values) == str:
                    if prefix in values:
                        item[keys] = os.path.join(directorydeterminer(),
                                                  values[values.rfind(prefix)+len(prefix):])
            ...
            for i in item:
                if type(i) == str:
                    if prefix in i:
                        nlst.append(os.path.join(directorydeterminer(),
                                                 i[i.rfind(prefix)+len(prefix):]))
                    else:
                        nlst.append(i)
                else:
                    nlst.append(i)
...
def dict_recursion(d, prefix):
    ...
    for k, v in d.items():
        if prefix in k:
            k = os.path.join(directorydeterminer(), k[k.rfind(prefix)+len(prefix):])
        if isinstance(v, dict):
            dict_recursion(v)
        else:
            if type(v) == str:
                if prefix in v:
                    v = os.path.join(directorydeterminer(), v[v.rfind(prefix)+len(prefix):])
        nd[k] = v
    return nd
...
def chunkit(core, cores, item_to_chunk):
    '''function used for parallel processes to determine the chunk range
    they should process, returns tuple of lower and upper ranges
    ...
    if cores == 1:
        ...
        chnkrng = (chnksz*(core)-1, len(item_to_chunk))
        # remainder for noneven chunking
    return chnkrng
...
def compress(src, compression=1, dst=False, cores=None):
    ...
    if cores == 1:
        if not dst:
            [tifffile.imsave(fl, tifffile.imread(fl), compress=compression)
             for fl in listdirfull(src) if fl[-4:] == '.tif']
        if dst:
            [tifffile.imsave(os.path.join(dst, fl[fl.rfind('/')+1:], compress=compression),
                             tifffile.imread(fl)) for fl in listdirfull(src) if fl[-4:] == '.tif']
    else:
        # paralellize
        sys.stdout.write('\n\nCompressing with {} cores. '.format(cores))
        sys.stdout.flush()
        jobs = listdirfull(src)
        joblength = range(len(jobs))
        iterlst = []
        [iterlst.append((jobs[jobid], compression, dst)) for jobid in joblength]
...
def compress_helper(fl, compression, dst):
    ...
    # different dst
        ...
        if fl[-4:] == '.tif':
            tifffile.imsave(os.path.join(dst, fl[fl.rfind('/')+1:]),
                            tifffile.imread(fl), compress=compression)
...
def compress_full_sizedatafld(jobid, cores=None, chunksize=None, compression=1, removeoriginal=False, **kwargs):
    ...
    pckl = open(os.path.join(outdr, 'param_dict.p'), 'rb')
    ...
    # find files
    fls = [os.path.join(xx, yy) for xx in [os.path.join(full_sizedatafld, 'ch{}'.format(x))
                                           for x in chs] for yy in os.listdir(xx)]
    fls.sort()
    ...
    start = time.time()
    iterlst = []
    [iterlst.append((jobid, fls_to_process, compression, removeoriginal, core, cores))
     for core in range(cores)]
    lst = p.starmap(compress_full_helper, iterlst)
    lst.sort()
    print('Completed {} through {}\nUsing {} cores'.format(
        fls_to_process[0], fls_to_process[-1], time.time() - start))
...
def dict_update(pth, old_system_directory=False):
    '''Function to update param dictionary by
    replacing both text of system direcotry
    ...
    elif type(old_system_directory) == str:
        old_system_directory = [old_system_directory]
...
def get_filepaths(directory):
    """
    # from: https://stackoverflow.com/questions/3207219/
    # how-to-list-all-files-of-a-directory-in-python/
    # 19308592#19308592?newreg=eda6052e44534b0982fb01506d1a2fbf
...
def listall(fld, keyword=False):
    ...
    if keyword:
        fls = [xx for xx in fls if keyword in xx]
    return fls


def listdirfull(x, keyword=False):
    '''might need to modify based on server...
    i.e. if automatically saving a file called 'thumbs'
    '''
    if not keyword:
        return [os.path.join(
            x, xx) for xx in os.listdir(x) if xx[0] != '.' and '~' not in xx
            and 'Thumbs.db' not in xx]
    else:
        return [os.path.join(x, xx) for xx in os.listdir(x)
                if xx[0] != '.' and '~' not in xx and 'Thumbs.db'
                not in xx and keyword in xx]
...
def load_tif_list(inn, cores=None, save=False):
    ...
    if cores:
        assert save, 'For parallelization to be efficient, you must save the memory mapped file, set save path'

    # if pth to folder
    if type(inn) == str:
        inn = [xx for xx in listdirfull(inn) if 'tif' in xx and '~' not in xx]
        inn.sort()
    ...
    # parallelized:
        ...
        arr = load_memmap_arr(save, dtype='uint16', mode='w+', shape=tuple(dims))
        del arr

        # paralellize
        p = mp.Pool(cores)
        iterlst = []
        [iterlst.append((core, cores, inn, save)) for core in range(cores)]
...
def load_tif_helper(core, cores, inn, save):
    ...
    print(
        '   chunk {} of {} - planes {}-{}\n'.format((
            core+1), (cores), chnkrng[0], chnkrng[1]))

    arr = np.lib.format.open_memmap(save, dtype='uint16', mode='r+')

    # load and save to memory mapped file
    # arr[chnkrng[0]:chnkrng[1],...] = np.squeeze
    # (tifffile.imread(inn[chnkrng[0]:chnkrng[1]]))
    for i in range(chnkrng[0], chnkrng[1]):
...
def log_step(task, out='complete', **kwargs):
    ...
    Assumes a dictionary as input.
    If dictionary contains k,v: 'dict_path'=path_to_save_location,
    then will automatically save
    '''
    if 'tasks' not in kwargs:
        kwargs['tasks'] = {}
    ...
    if 'dict_path' in kwargs:
        save_kwargs(kwargs['dict_path'], **kwargs)
...
def make_memmap_from_np_list(src, dst, cores=1, verbose=True):
    ...
    if verbose:
        sys.stdout.write(
            'Generating memory mapped array of dims {}:'.format(tuple(shape)))
        sys.stdout.flush()
    ...
    if cores <= 1 or cores == False:
        ...
        # init
        if verbose:
            sys.stdout.write('\n   loading {} of {}'.format(0, len(src)))
            sys.stdout.flush()
        arr = np.load(src[0])

        # init
        memmap = load_memmap_arr(
            dst, mode='w+', dtype=arr.dtype, shape=tuple(shape))
        ...
        p = mp.Pool(cores)
        iterlst = [(
            i, indices, shape, src, dst, verbose) for i in range(1, len(src))]
...
def norm(im, percentile=(0.1, 99.9),
         outdepth='uint16', zero_extremes=False, verbose=False):
    ...
    zero_extremes False: values outside upper
                  and lower percentiles become upper and lower
    ...
    im[im > high] = high_out
    im = (im - low) / (high - low)
    if verbose and percentile:
        print('{} pixel intensity at {} percentile, {} pixel intensity at {} percentile'.format(
            low, percentile[0], high, percentile[1]))
...
def resize(src, dst, zoomfactor=False, compression=1, cores=None):
    ...
    jobs = listdirfull(src)
    iterlst = []
    [iterlst.append((fl, dst, zoomfactor, compression)) for fl in listdirfull(src)
     if '.tif' in fl and '~' not in fl and 'Thumbs.db' not in fl]
    sys.stdout.write('{} files to compress...\n'.format(len(jobs)))
    sys.stdout.flush()
    parallel_process(iterlst, resize_helper, n_jobs=cores)
    sys.stdout.write('...done'.format(len(jobs)))
    sys.stdout.flush()
    return
...
def resize_helper(fl, dst, zoomfactor, compression):
    '''
    '''
    return tifffile.imsave(
        dst+fl[fl.rfind('/'):], zoom(
            tifffile.imread(
                fl, series=0, key=0), zoomfactor), compress=compression)
...
def resize_list(src, dst, zoomfactor=False, compression=1, cores=None):
    ...
    sys.stdout.flush()
    iterlst = []
    [iterlst.append((fl, dst, zoomfactor, compression))
     for fl in src if '.tif' in fl and '~' not in fl and 'Thumbs.db' not in fl]
    sys.stdout.write('{} files to compress...\n'.format(len(src)))
    sys.stdout.flush()
    parallel_process(iterlst, resize_helper, n_jobs=cores)
    sys.stdout.write('...done'.format(len(src)))
    sys.stdout.flush()
    return
...
def save_memmap_arr(pth, arr):
    ...
    '''
    narr = np.lib.format.open_memmap(
        pth, dtype=arr.dtype, shape=arr.shape, mode='w+')
    narr[:] = arr
    narr.flush()
    del narr
    return
...
def shape_np(src):
    ...
    with open(src, 'r') as f:
        s = f.readlines(1)
    s = s[0]
    ln = s[s.find('shape')+8:s.find('}')-2].replace(
        '(', '').replace(')', '').split(',')
...
def save_kwargs(pckloc=None, verbose=False, **kwargs):
    ...
    # handle input
    if not pckloc:
        pckloc = os.path.join(kwargs['outputdirectory'], 'param_dict.p')
...
def writer(saveloc, texttowrite, flnm=None, verbose=True):
    ...
    if verbose is True:
        if os.path.exists(saveloc) is False:
            ...
            return
        elif os.path.exists(saveloc) is True:
...
def uncompress(src, dst=False, cores=None):
    ...
    if cores == 1:
        if not dst:
            [tifffile.imsave(fl, tifffile.imread(fl))
             for fl in listdirfull(src) if fl[-4:] == '.tif']
        if dst:
            [tifffile.imsave(os.path.join(dst, fl[fl.rfind(
                '/')+1:]), tifffile.imread(fl))
                for fl in listdirfull(src) if fl[-4:] == '.tif']
        ...
        joblength = range(len(jobs))
        iterlst = []
        [iterlst.append((jobs[jobid], 0, dst)) for jobid in joblength]
        parallel_process(iterlst, compress_helper, n_jobs=cores)
