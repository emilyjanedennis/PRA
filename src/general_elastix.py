...
if __name__ == "__main__":
	...
	if len(sys.argv) > 6:
		try:
			if os.path.isdir(str(sys.argv[6])):
				...
				print("a sixth argument (src) was provided as {} but it is not a directory, proceeding with no src")
	...
	for pairnum in np.arange(0,len(mvtiffs)):
		...
		outputfilename = os.path.join(output_dir,"enlarged_tiffs/{}_for_{}.tif".format(mvtiff,fxtiff))
		print(outputfilename)
		outputdirectory = os.path.join(output_dir,"output_dirs/{}_to_{}".format(mvtiff,fxtiff))
