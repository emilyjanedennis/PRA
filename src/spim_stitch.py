...
def stitch_step2(volin):
    ...
    result1 = run(cmd1,shell=True,stdout=PIPE,stderr=PIPE)
    ...
    cmd3 = "terastitcher --placetiles --projin={} --projout={} --algorithm=MST".format(projin3,projout3) 
    print("running command: ",cmd3)

    result3 = run(cmd3,shell=True,stdout=PIPE,stderr=PIPE)
...
def stitch_step3(volin,volout):
    ...
    cmd = "terastitcher --merge --projin={} --volout={} --imout_depth=16 --resolutions=0".format(projin,volout) 
    print("running command: ",cmd)
...
if __name__ == "__main__":
    ...
    #####################################################
    print("step id is {}".format(stepid))
    if stepid == "step0":
